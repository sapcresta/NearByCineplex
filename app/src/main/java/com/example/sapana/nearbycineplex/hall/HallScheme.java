...
public class HallScheme {
    ...
    private Paint textPaint, backgroundPaint, markerPaint, scenePaint; //paint  class ko objects
    private int seatWidth, seatGap, offset;
    private int schemeBackgroundColor, unavailableSeatColor, chosenColor, sceneBackgroundColor,bookedSeatColor;
    private int selectedSeats, maxSelectedSeats;
    private Typeface typeface; //font
    private String sceneName; //screen ko name

    private Scene scene; //where is Scene class??
    private ZoomableImageView image;  //java class ko object
    private SeatListener listener;
    private MaxSeatsClickListener maxSeatsClickListener;
    //private List<Zone> zones;
    //private ZoneListener zoneListener;

    public HallScheme(ZoomableImageView image, Seat[][] seats, Context context) { //parametric constructor
        ...
        image.setClickListener(new ImageClickListener() {  //interface ko method override
            @Override
       ...
       // sceneName = context.getString(R.string.scene);
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            //sceneBackgroundColor = context.getResources().getColor(R.color.disabled_color);
           // availableSeatColor = context.getResources().getColor(R.color.holo_green_light);
            unavailableSeatColor = context.getResources().getColor(R.color.holo_red_light);
            bookedSeatColor=context.getResources().getColor(R.color.blue);
            chosenColor = context.getResources().getColor(R.color.holo_blue_dark);
            markerPaint = initTextPaint(context.getResources().getColor(R.color.black_gray));
            scenePaint = initTextPaint(context.getResources().getColor(R.color.black_gray)); //scene lekheko color
        } else {
            schemeBackgroundColor = context.getColor(R.color.light_grey); //main background
            unavailableSeatColor = context.getColor(R.color.holo_red_light);
            chosenColor = context.getColor(R.color.holo_blue_dark);
            bookedSeatColor=context.getResources().getColor(R.color.blue);
        ...
        backgroundPaint.setStrokeWidth(0);  //paint class ko methods

        seatWidth = 30;
        seatGap = 5;
        offset = 30;
        height = seats.length; //col
        width = seats[0].length; //row
    ...
    public void setBookedSeatColor(int color) {
        bookedSeatColor = color;
        image.setImageBitmap(getImageBitmap());
    }
...
/*    public void setZoneListener(ZoneListener zoneListener) {
    ...
    private void clickScheme(Point point) { //bujhena
   ...
   /* public boolean findZoneClick(Point p) {                    //not in use
        for (Zone zone : zones) {
    ...
    private Bitmap getImageBitmap() {
        ...
        Bitmap tempBitmap = Bitmap.createBitmap(bitmapWidth, bitmapHeight, Bitmap.Config.ARGB_8888); //ARGB ???
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if (seats[i][j].status() == SeatStatus.EMPTY)
                    continue;
                if (seats[i][j].status() == SeatStatus.INFO) {   //seat ko name position?
                ...
                else if (seats[i][j].status() == SeatStatus.BOOKED) {
                    backgroundPaint.setColor(bookedSeatColor);
                }else if (seats[i][j].status() == SeatStatus.FREE) {
                   backgroundPaint.setColor(seats[i][j].color());
                    //backgroundPaint.setColor(availableSeatColor);
                ...
                tempCanvas.drawRect(offset/2 + (seatWidth + seatGap) * j + scene.getLeftYOffset(),
                        ...
                        backgroundPaint);
                if (seats[i][j].status() == SeatStatus.CHOSEN) { // seat ko selected seat no.
    ...
    private void drawScene(Canvas canvas) {  //screen ko position,color ra text fix garna
    ...
    private void drawTextCentred(Canvas canvas, Paint paint, String text, float cx, float cy){ //  seat ko number
        paint.getTextBounds(text, 0, text.length(), textBounds); //number
        canvas.drawText(text, cx - textBounds.exactCenterX(), cy - textBounds.exactCenterY(), paint); //position
    }
    ...
    public void notifySeatListener(Seat s) { // yo vaneko select seat ra unselect seat display garna
        if (s.status() == SeatStatus.FREE) {
            if (listener != null) {
                //HallScheme.SeatStatus status=SeatStatus.BOOKED;
    ...
    public enum SeatStatus {
        FREE, BUSY, EMPTY, CHOSEN, INFO, BOOKED;
